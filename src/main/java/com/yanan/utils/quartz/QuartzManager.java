package com.yanan.utils.quartz;
...
public class QuartzManager implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent arg0) {
		Iterator<RegisterDefinition> iterator = PlugsFactory.getInstance().getAllRegisterDefinition().values().iterator();
		while (iterator.hasNext()) {
			ClassHelper helper = ClassHelper.getClassHelper(iterator.next().getRegisterClass());
			Cron cron;
			if (ReflectUtils.implementsOf(helper.getCacheClass(), org.quartz.Job.class)
	...
	public static class AbstractJobImpl implements Job {
		...
		public void execute(JobExecutionContext context) throws JobExecutionException {
			...
			try {
				Object jobInstance = PlugsFactory.getPluginsInstance(clz);
				Class<?>[] types = met.getParameterTypes();
				Object[] params = new Object[types.length];
				for(int i = 0;i<types.length;i++ ){
					if(ParameterUtils.isNotNullType(types[i])){
